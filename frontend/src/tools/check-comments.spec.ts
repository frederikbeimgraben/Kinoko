...
/** Legt eine Quelldatei unter `src/app` an, mit dem leeren `tools`-Ordner daneben. */
...
describe('check-comments', () => {
  it('meldet einen Verstoss mit Pfad und Grund', () => {
    ...
    try {
      ...
      expect(found[0].reason).toContain('zusammenhängende Zeilen');
    } finally {
  ...
  it('läuft ohne Verstoss durch', () => {
    const { root, allowPath } = fixture('export const x = 1; // in Ordnung\n');
    try {
      expect(report(root, allowPath)).toEqual([]);
    } finally {
      rmSync(root, { recursive: true, force: true });
