...
import numpy as np
import pytest
from manifest import CLASSES, histogram, schreibe
...
def pruefe(h: dict, low: float, high: float) -> None:
    ...
    assert h["classes"][0] == pytest.approx(low, abs=1e-6)
    assert h["classes"][-1] == pytest.approx(high, abs=1e-6)
    assert h["classes"] == sorted(h["classes"])
    assert min(h["shares"]) >= 0.0
...
def test_art_traegt_das_histogramm_an_der_woche(tmp_path):
    ...
    meta = {"name": "boletus_edulis", "top": top,
            "weeks": [{"year": 2026, "week": 36, "forecast": False,
                       "histogram": histogram(np.linspace(0, top, 500), 0.0, top)}]}
    pfad = tmp_path / "boletus_edulis.json"
    schreibe(pfad, meta)
    gelesen = json.loads(pfad.read_text())
    pruefe(gelesen["weeks"][0]["histogram"], 0.0, top)
...
def test_wochenebene_traegt_die_histogramme_neben_weeks(tmp_path):
    ...
    meta = {"layers": {"temperatur": {
        ...
        "histograms": {k: histogram(np.linspace(low, high, 500), low, high)
    ...
    for schluessel in ebene["weeks"]:
        pruefe(ebene["histograms"][schluessel], low, high)
...
def test_schreibe_haelt_zahlenlisten_auf_einer_zeile(tmp_path):
    ...
    meta = {"bounds": [[47.1, 4.9], [55.2, 15.1]], "zooms": [5, 8],
            "have": {"5": ["16/10", "16/11"]},
            "histogram": histogram(np.linspace(0, 1, 100), 0.0, 1.0)}
    schreibe(pfad, meta)
    text = pfad.read_text()
    ...
    for pfad in sorted(KARTEN.glob("*.json")):
        ...
        if pfad.name == "layers.json":
            for ebene in meta["layers"].values():
                if ebene.get("static"):
                    pruefe(ebene["histogram"], ebene["low"], ebene["high"])
                    continue
                for schluessel in ebene["weeks"]:
                    pruefe(ebene["histograms"][schluessel], ebene["low"], ebene["high"])
        elif "weeks" in meta:
            for woche in meta["weeks"]:
                pruefe(woche["histogram"], 0.0, meta["top"])
